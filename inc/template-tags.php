	...
	function understrap_posted_on() {
		...
		$posted_on = apply_filters(
			...
			sprintf(
				...
				esc_html_x( 'Posted on', 'post date', 'mrwoocommerce' ),
		...
		if ( 0 === $author_id ) {
			...
			$byline = apply_filters(
				...
				sprintf(
					...
					$posted_on ? esc_html_x( 'by', 'post author', 'mrwoocommerce' ) : esc_html_x( 'Posted by', 'post author', 'mrwoocommerce' ),
...
if ( ! function_exists( 'understrap_categories_list' ) ) {
	...
	function understrap_categories_list() {
		...
		if ( $categories_list && understrap_categorized_blog() ) {
			...
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %s', 'mrwoocommerce' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
}
...
if ( ! function_exists( 'understrap_tags_list' ) ) {
	...
	function understrap_tags_list() {
		...
		if ( $tags_list && ! is_wp_error( $tags_list ) ) {
			...
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %s', 'mrwoocommerce' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
}
...
if ( ! function_exists( 'understrap_comments_popup_link' ) ) {
	...
	function understrap_comments_popup_link() {
		...
		$leave_comment = sprintf(
			...
			__( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'mrwoocommerce' ),
			$post_title
		);
...
if ( ! function_exists( 'understrap_comment_navigation' ) ) {
	...
	function understrap_comment_navigation( $nav_id ) {
		...
		<nav class="comment-navigation" id="<?php echo esc_attr( $nav_id ); ?>">
			...
			<h1 class="screen-reader-text"><?php esc_html_e( 'Comments navigation', 'mrwoocommerce' ); ?></h1>

			<?php if ( get_previous_comments_link() ) { ?>
				<div class="nav-previous">
					<?php previous_comments_link( __( '&larr; Older Comments', 'mrwoocommerce' ) ); ?>
				</div>
			<?php } ?>

			<?php if ( get_next_comments_link() ) { ?>
				<div class="nav-next">
					<?php next_comments_link( __( 'Newer Comments &rarr;', 'mrwoocommerce' ) ); ?>
				</div>
			<?php } ?>
	...
	function understrap_edit_post_link() {
		edit_post_link(
			sprintf(
				...
				esc_html__( 'Edit %s', 'mrwoocommerce' ),
...
if ( ! function_exists( 'understrap_post_nav' ) ) {
	...
	function understrap_post_nav() {
		...
		<nav class="container navigation post-navigation">
			<h2 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'mrwoocommerce' ); ?></h2>
			<div class="d-flex nav-links justify-content-between">
				<?php
				if ( get_previous_post_link() ) {
					previous_post_link( '<span class="nav-previous">%link</span>', _x( '<i class="fa fa-angle-left"></i>&nbsp;%title', 'Previous post link', 'mrwoocommerce' ) );
				}
				if ( get_next_post_link() ) {
					next_post_link( '<span class="nav-next">%link</span>', _x( '%title&nbsp;<i class="fa fa-angle-right"></i>', 'Next post link', 'mrwoocommerce' ) );
				}
				?>
			</div><!-- .nav-links -->
		</nav><!-- .post-navigation -->
...
if ( ! function_exists( 'understrap_link_pages' ) ) {
	...
	function understrap_link_pages() {
		$args = array(
			'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'mrwoocommerce' ),
			'after'  => '</div>',
		);
...
if ( ! function_exists( 'understrap_get_list_item_separator' ) ) {
	...
	function understrap_get_list_item_separator() {
		...
		return esc_html__( ', ', 'mrwoocommerce' );
	}
}
